...
function registerBuiltInPlugins() {
    ...
    async execute({ message }, { llm }) {
      if (llm) {
        const result = await llm(
          ...
          null,
          [],
          'You are a Wikipedia search assistant. Return only the Wikipedia summary in 2-3 sentences, with a URL.',
          {}
        );
  ...
  registerPlugin('weather', {
    ...
    async execute({ message }) {
      const cityMatch =
        message.match(/weather\s+(?:in|at|for)\s+([A-Za-z\s]+?)(?:\?|$|\.|,)/i) ||
      ...
      try {
        ...
        if (!geoData.results || geoData.results.length === 0) {
          return {
            text: `I couldn't find a location matching "${city}". Try a more specific city name.`,
          };
        }
        ...
        if (daily && daily.time && daily.time.length > 1) {
          ...
          for (let i = 1; i < Math.min(daily.time.length, 4); i++) {
            ...
            parts.push(
              `**${dayName}**: ${cond}, ${min}–${max}°F (${prob > 20 ? prob + '% rain chance' : 'low rain chance'})`
            );
          }
          forecast = '\n\n**3-Day Forecast:**\n' + parts.join('\n');
        }
        ...
        const rainInfo =
          precip > 0
            ? `\n\nCurrently **raining** (${precip}mm precipitation).`
            : precip === 0
